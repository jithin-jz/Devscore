...
export default function DebtChart({ repos }) {
    ...
    const metrics = [
        ...
        { label: 'No Container', count: repos.filter(r => !r.has_docker).length, color: '#3b82f6' },
            ...
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ left: -20 }}>
                    ...
                    <YAxis
                        dataKey="name"
                        type="category"
                        tick={{ fill: 'var(--ds-muted)', fontSize: 8, fontWeight: 900, textTransform: 'uppercase' }}
                        width={80}
                        axisLine={false}
                        tickLine={false}
                    />
                    <Tooltip
                        contentStyle={{ backgroundColor: 'var(--ds-bg)', border: '1px solid var(--ds-border)', fontSize: '10px' }}
                    ...
                    <Bar dataKey="count" radius={[0, 4, 4, 0]} barSize={12}>
                        {chartData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={metrics[index].color} fillOpacity={0.4} />
                        ))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>

...
import { getTechRecommendations, dismissTechRecommendation, regenerateTechRecommendations } from '../lib/api';
import { Sparkles, ExternalLink, X, RefreshCw, Code2, Wrench, Cloud, BookOpen, Lightbulb } from 'lucide-react';

const CATEGORY_CONFIG = {
    language: { label: 'Language', icon: Code2, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
    framework: { label: 'Framework', icon: Wrench, color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20' },
    tool: { label: 'Tool', icon: Wrench, color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20' },
    cloud: { label: 'Cloud', icon: Cloud, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/20' },
    practice: { label: 'Practice', icon: Lightbulb, color: 'text-pink-400', bg: 'bg-pink-500/10', border: 'border-pink-500/20' },
};

const PRIORITY_STYLES = {
    high: { label: 'HIGH PRIORITY', bg: 'bg-ds-danger/10', text: 'text-ds-danger', border: 'border-ds-danger/20' },
    medium: { label: 'RECOMMENDED', bg: 'bg-ds-warning/10', text: 'text-ds-warning', border: 'border-ds-warning/20' },
    low: { label: 'NICE TO HAVE', bg: 'bg-ds-accent/10', text: 'text-ds-accent', border: 'border-ds-accent/20' },
};
...
export default function TechStackRecs() {
    ...
    const filteredRecs = activeFilter === 'all'
        ? techRecs
        : techRecs.filter(r => r.category === activeFilter);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-black text-ds-text uppercase tracking-tight">Tech Stack Recommendations</h3>
                        <p className="text-[10px] text-ds-muted">AI-powered learning path suggestions</p>
                    </div>
                </div>
                <button
            ...
            <div className="flex gap-2 flex-wrap">
                {filters.map(filter => (
                    <button
                        ...
                        className={`px-3 py-1 rounded text-[8px] font-black uppercase tracking-wider transition-all ${activeFilter === filter.key
            ...
            {filteredRecs.length === 0 ? (
                <div className="ds-panel py-16 text-center border-dashed border-ds-border/20">
                    ...
                    <h4 className="text-[11px] font-black text-ds-text uppercase opacity-40">No recommendations yet</h4>
                    <p className="text-[9px] text-ds-muted mt-2">Run an analysis to get personalized tech suggestions</p>
                </div>
            ) : (
                        ...
                        return (
                            <div key={rec.id} className="ds-panel group relative">
                                ...
                                <div className="flex items-start gap-4">
                                    <div className={`w-10 h-10 shrink-0 rounded-lg ${catConfig.bg} border ${catConfig.border} flex items-center justify-center`}>
                                        <CatIcon size={18} className={catConfig.color} />
                                    </div>
                                    <div className="flex-1 min-w-0 space-y-3">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <span className={`text-[7px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${priority.border} ${priority.bg} ${priority.text}`}>
                                                {priority.label}
                                            </span>
                                            <span className={`text-[7px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${catConfig.bg} ${catConfig.color}`}>
                                                {catConfig.label}
                                            </span>
                                        </div>

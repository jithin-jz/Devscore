...
export default function Dashboard() {
    ...
    return (
        <div className="min-h-screen bg-ds-bg text-ds-text selection:bg-ds-accent selection:text-ds-bg flex flex-col dot-grid">
            ...
            <Navbar>
                <nav className="hidden md:flex items-center bg-ds-accent/5 border border-ds-border rounded-lg p-0.5 ml-4">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`
                                flex items-center gap-2 px-4 py-1.5 rounded-md text-[9px] font-black uppercase tracking-wider transition-all
                                ${activeTab === tab.id
                                    ? 'bg-ds-accent text-ds-bg'
                                    : 'text-ds-muted hover:text-ds-text hover:bg-ds-accent/5'
                                }
                            `}
                        >
                            <tab.icon size={12} color={activeTab === tab.id ? "var(--ds-bg)" : "currentColor"} />
                            {tab.label}
                        </button>
                    ))}
                </nav>
            ...
            <main className="max-w-[1500px] mx-auto w-full px-6 py-8 flex-1 flex flex-col gap-8 animate-premium-fade-in">
                {activeTab === 'overview' && (
                        ...
                        {!hasScore && !analyzing ? (
                            <section className="ds-panel py-32 flex flex-col items-center justify-center text-center space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h2 className="text-lg font-black uppercase tracking-tight">Welcome to GitProof</h2>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                                ...
                                <div className="lg:col-span-9 space-y-8">
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                        <div className="ds-panel h-[380px] flex flex-col">
                    ...
                    <div className="space-y-12 animate-premium-fade-in">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mt-6">
                            ...
                            <div className="lg:col-span-4 space-y-8">
                                <div className="ds-panel p-8 space-y-12 relative overflow-hidden">
                            ...
                            <div className="lg:col-span-8 space-y-8">
                                ...
                                {recommendations.length > 0 ? (
                                    ...
                                    <div className="ds-panel py-40 text-center border-dashed border-white/10 flex flex-col items-center gap-4">
                                        <Sparkles className="text-ds-muted/20" size={40} />
                                        <h3 className="text-sm font-black text-white uppercase opacity-40">You're all caught up!</h3>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            <div className="lg:col-span-4 space-y-8">
                                <section className="ds-panel space-y-6">
                                    <div className="flex items-start gap-5">
                                        <img src={user?.avatar_url || 'https://github.com/ghost.png'} alt="" className="w-16 h-16 rounded-xl border border-ds-border" />
                                        <div className="space-y-1">
                                            <h3 className="text-xl font-black uppercase tracking-tight leading-none text-ds-text">{user?.github_username}</h3>
                                            <p className="text-[10px] text-ds-muted font-bold tracking-wider">{user?.email}</p>
                                            {user?.bio && <p className="text-[10px] text-ds-muted/80 leading-snug max-w-xs pt-1">{user.bio}</p>}
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-2 gap-4 py-4 border-y border-ds-border/50">
                                        <div>
                                            <span className="ds-label mb-1 block">Account Created</span>
                                            <span className="text-[10px] font-bold text-ds-text uppercase tracking-widest">
                                                {user?.created_at ? new Date(user.created_at).toLocaleDateString() : 'N/A'}
                                            </span>
                                        </div>
                                        <div>
                                            <span className="ds-label mb-1 block">Last Scanned</span>
                                            <span className="text-[10px] font-bold text-ds-text uppercase tracking-widest">
                                                {user?.last_analyzed ? new Date(user.last_analyzed).toLocaleDateString() : 'Never'}
                                            </span>
                                        </div>
                                    </div>
                                    <div className="pt-6 border-t border-ds-border/50">
                                        <button onClick={logout} className="w-full py-3 bg-ds-accent/5 text-ds-muted hover:text-ds-text hover:bg-ds-accent/10 text-[9px] font-black uppercase tracking-widest rounded-lg border border-ds-border transition-all flex items-center justify-center gap-2">
                            ...
                            <div className="lg:col-span-8 space-y-8">
                                <section className="ds-panel space-y-6">
                                    ...
                                    <div className="p-5 rounded-lg bg-ds-accent/5 border border-ds-border flex items-center justify-between">
                                        <div className="space-y-1">
                                        ...
                                        <button
                                            onClick={toggleTheme}
                                            className="px-4 py-2 bg-ds-accent text-ds-bg text-[9px] font-black uppercase tracking-widest rounded flex items-center gap-2 hover:brightness-90 transition-all"
                                        >
                                ...
                                <section className="ds-panel space-y-8 border-ds-danger/20">
                                    <div className="flex items-center gap-4">
                                        <ShieldCheckIcon className="text-ds-danger" size={24} color="#ef4444" />
                                        <div className="space-y-0.5">
                                    ...
                                    <div className="p-5 rounded-lg bg-ds-danger/[0.03] border border-ds-danger/10 flex items-center justify-between">
                                        <div className="space-y-1">
                                            <p className="text-[11px] font-black uppercase text-ds-text">Delete Account</p>
                                            <p className="text-[10px] font-medium text-ds-muted max-w-xs">Permanently delete your account and all associated data. This action cannot be undone.</p>
                                        </div>

                                        {!showConfirmDelete ? (
                                            <button
                                                onClick={() => setShowConfirmDelete(true)}
                                                className="px-4 py-2 bg-ds-danger/10 text-ds-danger text-[9px] font-black uppercase tracking-widest rounded border border-ds-danger/20 hover:bg-ds-danger hover:text-white transition-all"
                                            >
                                                Delete Account
                                            </button>
                                        ) : (
                                            <div className="flex gap-2">
                                                <button onClick={handleDeleteAccount} className="px-4 py-2 bg-ds-danger text-white text-[9px] font-black uppercase tracking-widest rounded shadow-lg shadow-ds-danger/20 hover:brightness-110 transition-all">Confirm Delete</button>
                                                <button onClick={() => setShowConfirmDelete(false)} className="px-4 py-2 bg-ds-accent/5 text-ds-text text-[9px] font-black uppercase tracking-widest rounded border border-ds-border hover:bg-ds-accent/10 transition-all">Cancel</button>
                                            </div>
                                        )}
                                    </div>
                                </section>
